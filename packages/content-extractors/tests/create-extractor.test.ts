import type { ContentMessage } from "@statewalker/content-blocks";
...
async function collect(gen: AsyncGenerator<ContentMessage>): Promise<ContentMessage[]> {
...
describe("createContentExtractor", () => {
  ...
  it("uses default registry when none provided (test with .md file)", async () => {
    const extract = createContentExtractor();
    const messages = await collect(extract(toByteStream("# Title\nbody"), { path: "doc.md" }));
  ...
  it("uses custom registry when provided", async () => {
    ...
    const messages = await collect(extract(toByteStream("data"), { path: "file.custom" }));
