import type { ContentMessage, ContentMessageProps } from "@statewalker/content-blocks";
...
function msg<P extends ReasonProps>(props: P, content = ""): ContentMessage & { props: P } {
...
export function reasonStop(error?: string): ReasonStopEvent {
  return msg({ type: "reason:stop", role: "system", time: now(), error }, error ?? "");
}
...
export function reasonTextDelta(delta: string): ReasonTextDeltaEvent {
  return msg({ type: "reason:text-delta", role: "assistant", time: now() }, delta);
}

export function reasonReasoningDelta(delta: string): ReasonReasoningDeltaEvent {
  return msg({ type: "reason:reasoning-delta", role: "assistant", time: now() }, delta);
}
