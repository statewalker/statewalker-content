import { ExtractorRegistry } from "@statewalker/content-extractors";
...
describe("ContentExtractorScanner", () => {
  ...
  it("skips files without extractor", async () => {
    ...
    const source = makeSource([{ uri: "/data/file.bin", stamp: new Date("2026-04-01T00:00:00Z") }]);
  ...
  it("handles removal cascade", async () => {
    ...
    await collect(
      scanner.scan(
        makeSource([{ uri: "/docs/readme.md", stamp: new Date("2026-04-01T00:00:00Z") }]),
      ),
    );
