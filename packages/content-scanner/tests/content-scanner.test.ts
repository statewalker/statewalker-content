import type { ContentSection } from "@statewalker/content-blocks";
...
async function collectEvents(gen: AsyncGenerator<ContentSection>): Promise<ContentSection[]> {
...
describe("ContentScanner", () => {
  ...
  describe("scan — detect added files", () => {
    it("detects all files as content-changed on first scan", async () => {
      ...
      const events = await collectEvents(scanner.scan({ collectionId: "docs" }));
  ...
  describe("scan — detect updates", () => {
    it("detects modified file as content-changed", async () => {
      ...
      // Modify the file
      await writeText(files, "/root/a.txt", "this is a much longer version of the content");
  ...
  describe("multiple collections", () => {
    it("scopes events to collection", async () => {
      ...
      const docEvents = await collectEvents(scanner.scan({ collectionId: "docs" }));
      ...
      const dataEvents = await collectEvents(scanner.scan({ collectionId: "data" }));
      ...
      const docEvents2 = await collectEvents(scanner.scan({ collectionId: "docs" }));
      expect(ofType(docEvents2, "content-removed")).toHaveLength(1);

      const dataEvents2 = await collectEvents(scanner.scan({ collectionId: "data" }));
