...
export function createContentManager(
  ...
  const manager: ContentManager = {
    async *sync(): AsyncGenerator<SyncEvent> {
      ...
      for await (const _event of extractor.scan(
        cascadeSource(filesStore, contentLast),
      )) {
      ...
      for await (const _event of splitter.scan(
        cascadeSource(contentStore, chunksLast),
      )) {
        // consumed — intermediate stage
      }
      ...
      for await (const event of ftsIndexer.scan(
        cascadeSource(chunksStore, ftsLast),
      )) {
    ...
    async search(params: ContentSearchParams): Promise<SearchHit[]> {
      ...
      const {
        queries,
        semanticQueries: _semanticQueries,
        topK = 10,
        paths,
        weights,
      } = params;
