 ...
 */
export async function* reasonLoop(
  params: ReasonLoopParams,
): AsyncGenerator<ReasonEvent> {
  const {
    getMessages,
    modelId,
    llm,
  ...
  try {
    ...
    while (true) {
      ...
      if (step >= maxIterations) {
        console.log(
          `[reason-loop] maxIterations (${maxIterations}) reached, stopping`,
        );
        break;
      }
    }
