...
describe("ContentVectorIndexerScanner", () => {
  ...
  it("indexes embeddings into the Index", async () => {
    ...
    await collect(
      scanner.scan(
        makeEmbeddingsSource([
          { uri: "/docs/readme.md", embeddings: [emb1, emb2] },
        ]),
      ),
    );
  ...
  it("removes documents from Index on cascade removal", async () => {
    ...
    await collect(
      scanner.scan(
        makeEmbeddingsSource([{ uri: "/docs/readme.md", embeddings: [emb] }]),
      ),
    );
    expect(documents.size).toBe(1);
  ...
  it("uses same block ID pattern as FTS indexer", async () => {
    ...
    await collect(
      scanner.scan(
        makeEmbeddingsSource([
          { uri: "/docs/readme.md", embeddings: [emb1, emb2, emb3] },
        ]),
      ),
    );
